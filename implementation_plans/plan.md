...
| 4 | `Wishlist` model + migration (`unique(user_id,product_id)` + FKs/indexes); `WishlistService`, `DuplicateWishlistItemException`, `StoreWishlistRequest`, `WishlistResource`; add/list/remove endpoints with ownership scoping + eager-load + atomic dup-prevention | `Add wishlist endpoints with ownership scoping and duplicate prevention` |
| 5 | Error-envelope normalisation in `bootstrap/app.php`; tighten validation; **full feature + unit suite** (isolation, 409, 404-not-leak, 401-JSON, 422) | `Add validation, error handling, and feature tests` |
| — | Reviewer subagent pass after Phase 5; apply fixes | `Address review findings` |
| 6a | `docker-compose` (app + MySQL 8, PHP 8.3 pinned) + GitHub Actions workflow running the suite on SQLite | `Add Docker setup and CI` |
| 6b | `README.md` (setup, env, migrate/seed, run, test; API docs with request/response examples; design-decisions incl. **real-inventory concurrency via `lockForUpdate` / atomic conditional `UPDATE` contrasted with the wishlist's constraint-based dup-prevention**) + `requests.http` + AI session transcript | `Add README, API docs, and AI session transcript` |
